...
struct Constants
{
    ...
    TypedBuffer<UniformBuffer> viewData;
};
...
void main(in payload Payload inPayload, out indices uint3 outTriangles[MAX_PRIMITIVE_COUNT], out vertices VertexOutput outVertices[MAX_VERTEX_COUNT], 
    uint dispatchThreadID : SV_DispatchThreadID, uint groupThreadId : SV_GroupThreadID)
{
    ...
    //{
    //    uint vertexIndex = groupThreadId + i * THREAD_GROUP_SIZE;
    //    vertexIndex = min(vertexIndex, meshlet.vertexCount - 1);
        
    //    const float3 vertexPosition = mesh.vertexPositionsBuffer.Load(vertexIndex).position;
