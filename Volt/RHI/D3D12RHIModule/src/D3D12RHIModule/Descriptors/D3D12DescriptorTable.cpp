...
namespace Volt::RHI
{
	...
	void D3D12DescriptorTable::SetImageView(WeakPtr<ImageView> imageView, uint32_t set, uint32_t binding, ShaderRegisterType registerType)
	{
		...
		{
			VT_LOGC(LogSeverity::Warning, "[D3D12DescriptorTable]", "Trying to assign image view at set {0} and binding {1}. But that is not a valid binding!", set, binding);
			return;
		}
		...
		{
			VT_LOGC(LogSeverity::Error, "[D3D12DescriptorTable]", "Image View does not support the required D3D12 view type!");
			return;
		}
	...
	void D3D12DescriptorTable::SetBufferView(WeakPtr<BufferView> bufferView, uint32_t set, uint32_t binding, ShaderRegisterType registerType)
	{
		if (!m_allocatedDescriptorPointers[set].contains(binding))
		{
			VT_LOGC(LogSeverity::Warning, "[D3D12DescriptorTable]:", "Trying to assign buffer view at set {0} and binding {1}. But that is not a valid binding!", set, binding);
			return;
		}
		...
		if (!EnumValueContainsFlag(d3d12View.GetD3D12ViewType(), descriptorInfo.viewType))
		{
			VT_LOGC(LogSeverity::Error, "[D3D12DescriptorTable]:", "Buffer View does not support the required D3D12 view type!");
			return;
		}
	...
	void D3D12DescriptorTable::SetSamplerState(WeakPtr<SamplerState> samplerState, uint32_t set, uint32_t binding, ShaderRegisterType registerType)
	{
		if (!m_allocatedDescriptorPointers[set].contains(binding))
		{
			VT_LOGC(LogSeverity::Warning, "[D3D12DescriptorTable]:", "Trying to assign sampler state at set {0} and binding {1}. But that is not a valid binding!", set, binding);
			return;
		}
