...
namespace Volt::RHI
{
	...
	{
		VT_LOGC(LogSeverity::Trace, "VulkanRHIModule", "Initializing VulkanShaderCompiler");
	...
	VulkanShaderCompiler::~VulkanShaderCompiler()
	{
		...
		VT_LOGC(LogSeverity::Trace, "VulkanRHIModule", "Destroying VulkanShaderCompiler");
	}
	...
	ShaderCompiler::CompilationResultData VulkanShaderCompiler::TryCompileImpl(const Specification& specification)
	{
		...
		{
			VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "Trying to compile a shader without sources!");
			return {};
		}
	...
	ShaderCompiler::CompilationResult VulkanShaderCompiler::CompileSingle(ShaderStage shaderStage, const std::string& source, const ShaderSourceEntry& sourceEntry, const Specification& specification, CompilationResultData& outData)
	{
		...
		// Pre processing
		{
			...
			else if (result.renderGraphConstants.IsValid())
			{
				for (const auto& [name, uniform] : outData.renderGraphConstants.uniforms)
				{
					...
					{
						VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "All shader stages must have equal constant struct definition!");
					}
				}
			}
		...
		if (error.empty())
		{
			...
			if (!shaderResult || shaderResult->GetBufferSize() == 0)
			{
				...
				VT_LOGC(LogSeverity::Error, "VulkanRHIModule", error);
	...
	void VulkanShaderCompiler::ReflectAllStages(const Specification& specification, CompilationResultData& inOutData)
	{
		...
		{
			VT_LOGC(LogSeverity::Trace, "VulkanRHIModule", "Reflecting shader {0}", specification.shaderSourceInfo.at(stage).sourceEntry.filePath.string());
	...
	void VulkanShaderCompiler::ReflectStage(ShaderStage stage, const Specification& specification, CompilationResultData& inOutData)
	{
		...
		for (const auto& ubo : resources.uniform_buffers)
		{
			...
			if (name == "$Globals")
			{
				VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "Shader {0} seems to have incorrectly defined global variables!", specification.shaderSourceInfo.at(stage).sourceEntry.filePath.string());
				continue;
			}
