...
namespace Volt::RHI
{
	...
	RefPtr<Allocation> VulkanTransientHeap::CreateBuffer(const TransientBufferCreateInfo& createInfo)
	{
		...
		{
			VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "Unable to find available allocation block for buffer allocation of size {0}!", createInfo.size);
			return nullptr;
		}
	...
	RefPtr<Allocation> VulkanTransientHeap::CreateImage(const TransientImageCreateInfo& createInfo)
	{
		...
		if (blockAlloc.size == 0)
		{
			VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "Unable to find available allocation block for image allocation of size {0}!", createInfo.size);
			return nullptr;
		}
	...
	void VulkanTransientHeap::InitializeAsBufferHeap()
	{
		...
		{
			VT_LOGC(LogSeverity::Error, "VulkanRHIModule", "Unable to find memory type index from bits {0}!", m_memoryRequirements.memoryTypeBits);
			return;
		}
