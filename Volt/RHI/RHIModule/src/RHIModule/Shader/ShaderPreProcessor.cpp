...
namespace Volt::RHI
{
	...
	bool ShaderPreProcessor::PreProcessPixelSource(const PreProcessorData& data, PreProcessorResult& outResult)
	{
		...
		{
			VT_LOGC(LogSeverity::Error, "ShaderPreProcessor", "Unable to find Entry Point {0} in shader!", entryPoint);
			return false;
		}
	...
	PixelFormat ShaderPreProcessor::FindDefaultFormatFromString(std::string_view str)
	{
		...
		VT_LOGC(LogSeverity::Error, "ShaderPreProcessor", "Unable to translate type {0} into any format!", str);
	...
	PixelFormat ShaderPreProcessor::FindFormatFromLayoutQualifier(const std::string& layoutStr)
	{
		...
		VT_LOGC(LogSeverity::Error, "ShaderPreProcessor", "Unable to translate layout qualifier {0} into any format!", tempStr);
		return PixelFormat::UNDEFINED;
	}
}
