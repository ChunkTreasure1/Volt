	...
	switch (e.GetKeyCode())
	{
		...
		case VT_KEY_G:
		{
			if (!ctrlPressed)
			{
				...
				m_sceneRenderer->GetSettings().enableGrid = UserSettingsManager::GetSettings().sceneSettings.gridEnabled;
			}
			break;
		}
...
void ViewportPanel::HandleSingleSelect()
{
	...
	if (mouseX >= 0 && mouseY >= 0 && mouseX < (int32_t)perspectiveSize.x && mouseY < (int32_t)perspectiveSize.y)
	{
		...
		uint32_t pixelData = m_sceneRenderer->GetIDImage()->ReadPixel<uint32_t>(static_cast<uint32_t>(mouseX * renderScale), static_cast<uint32_t>(mouseY * renderScale));
...
void ViewportPanel::HandleMultiSelect()
{
	...
	const glm::vec2 startDragPos = GetViewportLocalPosition(m_startDragPos);
