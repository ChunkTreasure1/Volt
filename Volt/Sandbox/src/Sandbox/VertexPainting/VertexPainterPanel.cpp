		...
		if (paintedEnt.HasComponent<Volt::MeshComponent>())
		{
			...
			for (auto& submesh : mesh->GetSubMeshes())
			{
				for (uint32_t index = submesh.vertexStartOffset; index < submesh.vertexStartOffset + submesh.vertexCount; index++)
				{
					...
					gem::vec4 vertexColor = hasPainted ? Volt::Utility::UnpackUIntToUNormFloat4(paintedEnt.GetComponent<Volt::VertexPaintedComponent>().vertexColors[index]) : 0.f;
		...
		for (auto& submesh : mesh->GetSubMeshes())
		{
			for (uint32_t index = submesh.vertexStartOffset; index < submesh.vertexStartOffset + submesh.vertexCount; index++)
			{
				...
				if (gem::distance2(vPos, m_brushPosition) < m_settings.billboardRange * m_settings.billboardRange)
				{
					...
					gem::vec4 unpackedColor = Volt::Utility::UnpackUIntToUNormFloat4(currentVertexColor);
