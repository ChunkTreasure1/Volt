...
namespace V015
{
	...
	void Convert()
	{
		...
		{
			importers.emplace(Volt::AssetType::Shader, CreateScope<Volt::ShaderImporter>()); 
			importers.emplace(Volt::AssetType::TextureSource, CreateScope<Volt::TextureSourceImporter>()); 
			importers.emplace(Volt::AssetType::Material, CreateScope<Volt::MaterialImporter>());
		...
		{
			if (!importers.contains(metadata.type) || metadata.type == Volt::AssetType::Shader || metadata.type == Volt::AssetType::TextureSource)
			{
				continue;
			}
