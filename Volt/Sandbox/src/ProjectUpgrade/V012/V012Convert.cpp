	...
	inline void ConvertAnimationGraphsToV012()
	{
		...
		for (const auto& animGraphPath : animGraphsToConvert)
		{
			...
			{ // convert the animgraph
				...
				if (!input.is_open())
				{
					VT_LOG(LogSeverity::Error, "Failed to convert: File {0} not found!", animGraphPath);
					continue;
				}
		...
		for (auto& layer : layerAssets)
		{
			...
			if (!input.is_open())
			{
				VT_LOG(LogSeverity::Error, "Failed to convert: File {0} not found!", layer);
				continue;
			}
		...
		for (auto& prefab : prefabAssets)
		{
			...
			if (!input.is_open())
			{
				VT_LOG(LogSeverity::Error, "Failed to convert: File {0} not found!", prefab);
				continue;
			}
