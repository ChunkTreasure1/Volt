...
namespace Volt::RHI
{
	...
	RefPtr<Allocation> D3D12TransientHeap::CreateBuffer(const TransientBufferCreateInfo& createInfo)
	{
		...
		{
			RHILog::LogTagged(LogSeverity::Error, "[D3D12TransientHeap]", "Unable to find available allocation block for buffer allocation of size {0}!", createInfo.size);
			return nullptr;
		}
	...
	RefPtr<Allocation> D3D12TransientHeap::CreateImage(const TransientImageCreateInfo& createInfo)
	{
		...
		if (blockAlloc.size == 0)
		{
			RHILog::LogTagged(LogSeverity::Error, "[D3D12TransientHeap]", "Unable to find available allocation block for image allocation of size {0}!", createInfo.size);
			return nullptr;
		}
