	...
	namespace AI
	{
		...
		void NavigationSystem::SyncDetourPosition(Volt::Entity entity, float deltaTime)
		{
			...
			if (entity.HasComponent<Volt::CharacterControllerComponent>())
			{
				...
				else
				{
					...
					{
						VT_LOG(LogSeverity::Error, "No valid actor controller found for entity {0}!", entity.GetID());
					}
				}
			}
			else
		...
		void NavigationSystem::InitAgents()
		{
			if (myNavMesh)
			{
				...
				if (!myActiveScene)
				{
					VT_LOG(LogSeverity::Error, "Could not initialize agents because active scene is null");
					return;
				}
