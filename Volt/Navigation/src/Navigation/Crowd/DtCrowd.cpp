...
namespace Volt
{
	namespace AI
	{
			...
			: myNavMesh(navmesh)
		{
			...
			{
				VT_LOG(LogSeverity::Error, "Could not init DtCrowd");
			}
		...
		const dtCrowdAgent* DtCrowd::GetAgent(Volt::Entity entity)
		{
			...
			{
				VT_LOG(LogSeverity::Warning, "Could not get agent from entity: {0}", entity.GetID());
				return nullptr;
			}
		...
		const dtCrowdAgent* DtCrowd::GetAgent(EntityID entityId)
		{
			...
			{
				VT_LOG(LogSeverity::Warning, "Could not get agent from entity: {0}", entityId);
				return nullptr;
			}
		...
		void DtCrowd::SetAgentPosition(Volt::Entity entity, glm::vec3 position)
		{
			...
			if (!myCrowd || !myEntityToAgentMap.contains(entityId))
			{
				VT_LOG(LogSeverity::Warning, "Could not set agent position for entity: {0}", entityId);
				return;
			}
		...
		void DtCrowd::SetAgentTarget(Volt::Entity entity, glm::vec3 target)
		{
			...
			{
				VT_LOG(LogSeverity::Warning, "Could not set agent target for entity: {0}", entityId);
				return;
			}
		...
		void DtCrowd::UpdateAgentParams(Volt::Entity entity)
		{
			if (!myCrowd)
			{
				VT_LOG(LogSeverity::Warning, "Could not update agent parameters for entity: {0}", entity.GetID());
				return;
			}
		...
		void DtCrowd::AddAgent(Volt::Entity entity)
		{
			if (!myCrowd || !entity.HasComponent<Volt::NavAgentComponent>() || myEntityToAgentMap.contains(entity.GetID()))
			{
				VT_LOG(LogSeverity::Warning, "Could not add agent for entity: {0}", entity.GetID());
				return;
			}
