...
namespace Volt
{
	namespace AI
	{
			...
			: myNavMesh(navmesh), myTileCache(tilecache)
		{
			...
			{
				VT_LOG(LogVerbosity::Error, "Could not init Detour NavMesh Query");
			}
		}
		...
		Vector<glm::vec3> DtNavMesh::FindPath(glm::vec3 start, glm::vec3 end, glm::vec3 polySearchDistance)
		{
			...
			if (dtStatusFailed(status))
			{
				VT_LOG(LogVerbosity::Warning, "Detour failed to find nearest start poly");
				return resultPath;
			}
			...
			if (dtStatusFailed(status))
			{
				VT_LOG(LogVerbosity::Warning, "Detour failed to find nearest end poly");
				return resultPath;
			}
			...
			{
				VT_LOG(LogVerbosity::Warning, "Detour failed to find path");
				return resultPath;
			}
			...
			if (dtStatusFailed(status))
			{
				VT_LOG(LogVerbosity::Warning, "Detour failed to find straight path");
				return resultPath;
			}
