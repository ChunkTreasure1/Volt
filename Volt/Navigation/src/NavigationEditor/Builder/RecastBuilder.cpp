							   ...
							   const int maxTiles)
{
	...
	{
		VT_LOG(LogVerbosity::Error, "buildTile: Input mesh is not specified.");
		return 0;
	}
	...
	if (!rc.solid)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'solid'.");
		return 0;
	}
	if (!rcCreateHeightfield(m_ctx.get(), *rc.solid, tcfg.width, tcfg.height, tcfg.bmin, tcfg.bmax, tcfg.cs, tcfg.ch))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not create solid heightfield.");
		return 0;
	}
	...
	if (!rc.triareas)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'm_triareas' ({0}).", chunkyMesh->maxTrisPerChunk);
		return 0;
	}
	...
	if (!rc.chf)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'chf'.");
		return 0;
	}
	if (!rcBuildCompactHeightfield(m_ctx.get(), tcfg.walkableHeight, tcfg.walkableClimb, *rc.solid, *rc.chf))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build compact data.");
		return 0;
	}
	...
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not erode.");
		return 0;
	}
	...
	if (!rc.lset)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'lset'.");
		return 0;
	}
	if (!rcBuildHeightfieldLayers(m_ctx.get(), *rc.chf, tcfg.borderSize, tcfg.walkableHeight, *rc.lset))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build heighfield layers.");
		return 0;
	}
...
Ref<Volt::AI::NavMesh> RecastBuilder::BuildSingleNavMesh()
{
	...
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Input mesh is not specified.");
		return nullptr;
	}
	...
	VT_LOG(LogVerbosity::Info, "Building navigation:");
	VT_LOG(LogVerbosity::Info, " - {0} x {1} cells", m_cfg.width, m_cfg.height);
	VT_LOG(LogVerbosity::Info, " - {0}K verts, {1}K tris", nverts / 1000.0f, ntris / 1000.0f);
	...
	if (!m_triareas)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'm_triareas' ({0}).", ntris);
		return nullptr;
	}
	...
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not rasterize triangles.");
		return nullptr;
	}
	...
	if (m_buildSettings->partitionType == PARTITION_WATERSHED)
	{
		...
		{
			VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build distance field.");
			return nullptr;
		}
		...
		{
			VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build watershed regions.");
			return nullptr;
		}
	}
	else if (m_buildSettings->partitionType == PARTITION_MONOTONE)
	{
		...
		{
			VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build monotone regions.");
			return nullptr;
		}
	}
	else if (m_buildSettings->partitionType == PARTITION_LAYERS)
	{
		...
		{
			VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build layer regions.");
			return nullptr;
		}
	}
	...
	if (!m_cset)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'cset'.");
		return nullptr;
	}
	if (!rcBuildContours(m_ctx.get(), *m_chf, m_cfg.maxSimplificationError, m_cfg.maxEdgeLen, *m_cset))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not create contours.");
		return nullptr;
	}
	...
	if (!m_pmesh)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'pmesh'.");
		return nullptr;
	}
	if (!rcBuildPolyMesh(m_ctx.get(), *m_cset, m_cfg.maxVertsPerPoly, *m_pmesh))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not triangulate contours.");
		return nullptr;
	}
	...
	if (!m_dmesh)
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Out of memory 'pmdtl'.");
		return nullptr;
	}

	if (!rcBuildPolyMeshDetail(m_ctx.get(), *m_pmesh, *m_chf, m_cfg.detailSampleDist, m_cfg.detailSampleMaxError, *m_dmesh))
	{
		VT_LOG(LogVerbosity::Error, "buildNavigation: Could not build detail mesh.");
		return nullptr;
	}
	...
	if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
	{
		...
		{
			VT_LOG(LogVerbosity::Error, "Could not build Detour navmesh.");
			return nullptr;
		}
		...
		{
			dtFree(navData);
			VT_LOG(LogVerbosity::Error, "Could not create Detour navmesh");
			return nullptr;
		}
		...
		{
			dtFree(navData);
			VT_LOG(LogVerbosity::Error, "Could not init Detour navmesh");
			return nullptr;
		}
	...
	VT_LOG(LogVerbosity::Info, ">> Detailed Polymesh: {0} vertices, {1} triangles", m_dmesh->nverts, m_dmesh->ntris);
	VT_LOG(LogVerbosity::Info, ">> Polymesh: {0} vertices, {1} polygons", m_pmesh->nverts, m_pmesh->npolys);

	m_totalBuildTimeMs = m_ctx->getAccumulatedTime(RC_TIMER_TOTAL) / 1000.0f;
	VT_LOG(LogVerbosity::Info, ">> NavMesh finished building in {0}ms", m_totalBuildTimeMs);
...
Ref<Volt::AI::NavMesh> RecastBuilder::BuildTiledNavMesh()
{
	if (!m_geom || !m_geom->getVertCount())
	{
		VT_LOG(LogVerbosity::Error, "buildTiledNavigation: Input mesh is not specified.");
		return nullptr;
	}
	...
	if (!tileCache)
	{
		VT_LOG(LogVerbosity::Error, "buildTiledNavigation: Could not allocate tile cache.");
		return nullptr;
	}
	...
	if (dtStatusFailed(status))
	{
		VT_LOG(LogVerbosity::Error, "buildTiledNavigation: Could not init tile cache.");
		return nullptr;
	}
	...
	if (!navMesh)
	{
		VT_LOG(LogVerbosity::Error, "buildTiledNavigation: Could not allocate navmesh.");
		return nullptr;
	}
	...
	if (dtStatusFailed(status))
	{
		VT_LOG(LogVerbosity::Error, "buildTiledNavigation: Could not init navmesh.");
		return nullptr;
	}
	...
	// Show performance stats.
	VT_LOG(LogVerbosity::Info, ">> Layers: {0}", m_cacheLayerCount);
	VT_LOG(LogVerbosity::Info, ">> Layers (per tile) {0}", (float)m_cacheLayerCount / (float)gridSize);
	VT_LOG(LogVerbosity::Info, ">> Memory: {0} kB / {1} kB ({2})", m_cacheCompressedSize / 1024.0f, m_cacheRawSize / 1024.0f, compressionRatio * 100.0f);
	VT_LOG(LogVerbosity::Info, ">> Build Peak Mem Usage: {0} kB", m_cacheBuildMemUsage / 1024.0f);
	VT_LOG(LogVerbosity::Info, ">> NavMesh finished building in {0}ms", m_cacheBuildTimeMs);
