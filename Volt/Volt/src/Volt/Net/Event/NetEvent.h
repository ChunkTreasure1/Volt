...
namespace Volt
{
	...
	{
	public:
		NetEvent(eNetEvent in_event, Nexus::TYPE::REP_ID in_id, Vector<uint8_t> in_data) : m_event(in_event), m_id(in_id), m_data(in_data) {}
		NetEvent() {}
		~NetEvent() {}
		...
		void SetData(Vector<uint8_t> in_data) { m_data = in_data; }
