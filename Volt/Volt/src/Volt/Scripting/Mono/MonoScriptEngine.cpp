...
namespace Volt
{
	...
	void MonoScriptEngine::OnAwakeInstance(UUID64 instanceId, EntityID entity, const std::string& fullClassName)
	{
		...
		for (const auto& [name, fieldInstance] : fieldMap)
		{
			if (fieldInstance->field.type.IsEntity())
			{
				...
				if (fieldEnt == Entity::NullID() || !s_monoData->sceneContext->IsEntityValid(fieldEnt))
				{
					continue;
				}
	...
	void MonoScriptEngine::OnCreateInstance(UUID64 instanceId, EntityID entity, const std::string& fullClassName)
	{
	...
	void MonoScriptEngine::SetScriptFieldDefaultData(UUID64 instanceId, EntityID entity, const std::string& fullClassName)
	{
		if (!EntityClassExists(fullClassName))
		{
			return;
