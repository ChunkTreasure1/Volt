...
bool Volt::TimelineImporter::Load(const std::filesystem::path& path, Ref<Asset>& asset) const
{
	...
	for (auto track : root["Tracks"])
	{
		...
		{
			VT_DESERIALIZE_PROPERTY(targetEntId, newTrack.targetEntity, track, 0u);
		...
		else if (newTrack.trackType == TrackType::T_Clip)
		{
			for (auto clip : track["Clips"])
			{
				...
				VT_DESERIALIZE_PROPERTY(clipCamera, newClip.activeCamera, clip, 0u);
...
void Volt::TimelineImporter::Save(const Ref<Asset>& asset) const
{
	Ref<TimelinePreset> preset = std::reinterpret_pointer_cast<TimelinePreset>(asset);
...
void Volt::TimelineImporter::SaveBinary(uint8_t* buffer, const Ref<Asset>& asset) const
{
}

bool Volt::TimelineImporter::LoadBinary(const uint8_t* buffer, const AssetPacker::AssetHeader& header, Ref<Asset>& asset) const
{
	return false;
}
