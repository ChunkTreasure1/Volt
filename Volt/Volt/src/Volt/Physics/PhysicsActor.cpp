...
namespace Volt
{
	...
	void PhysicsActor::SetLinearDrag(float drag) const
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Trying to set drag of non-dynamic PhysicsActor!");
			return;
		}
	...
	void PhysicsActor::SetMass(float mass)
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Static PhysicsActor can't have mass!");
			return;
		}
	...
	void PhysicsActor::SetKinematic(bool isKinematic)
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Static PhysicsActor can't be kinematic!");
			return;
		}
	...
	void PhysicsActor::SetKinematicTarget(const glm::vec3& position, const glm::quat& rotation)
	{
		if (!IsKinematic())
		{
			VT_LOG(LogSeverity::Warning, "Trying to set kinematic target for a non-kinematic actor.");
			return;
		}
	...
	void PhysicsActor::SetLinearVelocity(const glm::vec3& velocity)
	{
		...
		{
			VT_LOG(LogSeverity::Warning, "Trying to set velocity of non-dynamic PhysicsActor");
			return;
		}
	...
	const glm::vec3 PhysicsActor::GetLinearVelocity() const
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Trying to get velocity of non-dynamic PhysicsActor");
			return glm::vec3(0.f);
		}
	...
	void PhysicsActor::AddForce(const glm::vec3& aForce, ForceMode aForceMode)
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Trying to add force to non-dynamic PhysicsActor");
			return;
		}
	...
	void PhysicsActor::AddTorque(const glm::vec3& torque, ForceMode aForceMode)
	{
		if (!IsDynamic())
		{
			VT_LOG(LogSeverity::Warning, "Trying to add torque to non-dynamic PhysicsActor");
			return;
		}
