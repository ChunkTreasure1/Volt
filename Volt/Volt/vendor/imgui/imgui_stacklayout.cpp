...
static void ImGui::BeginLayoutItem(ImGuiLayout& layout)
{
    ...
    if (item.CurrentAlign < 0.0f)
        item.CurrentAlign = ImClamp(/*g.Style.LayoutAlign*/ 0.5f, 0.0f, 1.0f); //g.style.LayoutAlign had a default value of 0.5f in the original code, but since we are now using the external stacklayout api, we don't have access to the style struct anymore
