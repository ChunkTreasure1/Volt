...
GLFWAPI GLFWtitlebarhittestfun glfwSetTitlebarHitTestCallback(GLFWwindow* handle, GLFWtitlebarhittestfun callback)
{
	_GLFWwindow* window = (_GLFWwindow*)handle;
	assert(window != NULL);

	_GLFW_REQUIRE_INIT_OR_RETURN(NULL);
	_GLFW_SWAP(GLFWtitlebarhittestfun, window->callbacks.tbhittest, callback);
	return callback;
}
